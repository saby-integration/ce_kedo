...
// BSLLS:FieldsFromJoinsWithoutIsNull-on	
	...
	|	ЕСТЬNULL(КадроваяИсторияСотрудниковСрезПоследних.Подразделение, ЗНАЧЕНИЕ(Справочник.Подразделения.ПустаяСсылка)) КАК Подразделение,
	...
	|	ВТ КАК ВТ
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КадровыеСведения.СрезПоследних(&НаДату, ) КАК КадроваяИсторияСотрудниковСрезПоследних
	|		ПО (ВТ.Организация = КадроваяИсторияСотрудниковСрезПоследних.Организация)
	...
	// Данная магия програмного добавления нужна, для того, что бы не захватывать 
	// все дополнительные справочнеики используемыве в запросе
