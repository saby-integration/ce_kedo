...
#Область Saby_ФоновыеЗаданияСервер_ПараметрыСеанса

&НаСервере
Функция ПараметрыСеансаРасширения() Экспорт
	СеансПараметры = Новый Структура;
	СеансПараметры.Вставить("Параметры", ПараметрыСеанса.РасширениеФоновыеЗаданияПараметры.Получить());
	СеансПараметры.Вставить("ДополнительныеПараметры", ПараметрыСеанса.РасширениеФоновыеЗаданияДополнительныеПараметры.Получить());
	СеансПараметры.Вставить("Статус", ПараметрыСеанса.РасширениеФоновыеЗаданияСтатус);
	СеансПараметры.Вставить("ИдентификаторЗадания", ПараметрыСеанса.РасширениеФоновыеЗаданияИдентификаторЗадания);
	СеансПараметры.Вставить("ТекстОповещения", ПараметрыСеанса.РасширениеФоновыеТекстОповещения);
	СеансПараметры.Вставить("ИнтервалОжидания", ПараметрыСеанса.РасширениеФоновыеЗаданияИнтервалОжидания);
	СеансПараметры.Вставить("КартинкаОповещения", ПараметрыСеанса.РасширениеФоновыеЗаданияКартинкаОповещения);
	Возврат СеансПараметры;
КонецФункции

&НаСервере
Процедура ПараметрыСеансаУстановитьИнтервалОжидания(Интервал) Экспорт
	ПараметрыСеанса.РасширениеФоновыеЗаданияИнтервалОжидания = Интервал;
КонецПроцедуры

&НаСервере
Процедура ПараметрыСеансаУстановитьСтатус(Статус) Экспорт
	ПараметрыСеанса.РасширениеФоновыеЗаданияСтатус = Статус;
КонецПроцедуры

&НаСервере
Процедура ПараметрыСеансаУстановитьПараметры(Параметры) Экспорт
	ПараметрыСеанса.РасширениеФоновыеЗаданияПараметры = Новый ХранилищеЗначения(Параметры);
КонецПроцедуры

&НаСервере
Процедура ПараметрыСеансаУстановитьДополнительныеПараметры(ДополнительныеПараметры) Экспорт
	ПараметрыСеанса.РасширениеФоновыеЗаданияДополнительныеПараметры = Новый ХранилищеЗначения(ДополнительныеПараметры);
КонецПроцедуры

&НаСервере
Процедура ПараметрыСеансаУстановитьИдентификаторЗадания(ИдентификаторЗадания) Экспорт
	ПараметрыСеанса.РасширениеФоновыеЗаданияИдентификаторЗадания = ИдентификаторЗадания;
КонецПроцедуры

&НаСервере
Процедура ПараметрыСеансаУстановитьКартинкуОповещения(ИмяКартинки) Экспорт
	ПараметрыСеанса.РасширениеФоновыеЗаданияКартинкаОповещения = ИмяКартинки;
КонецПроцедуры

&НаСервере
Процедура ПараметрыСеансаУстановитьТекстОповещения(ТекстОповещения) Экспорт
	ПараметрыСеанса.РасширениеФоновыеТекстОповещения = ТекстОповещения;
КонецПроцедуры

&НаСервере
Процедура ПараметрыСеансаСброс(Параметры = Неопределено) Экспорт
	Если Параметры = Неопределено Тогда
		Параметры = Новый Структура;
	КонецЕсли;
	ПараметрыСеанса.РасширениеФоновыеЗаданияПараметры = Новый ХранилищеЗначения(Параметры);
	ПараметрыСеанса.РасширениеФоновыеЗаданияДополнительныеПараметры = Новый ХранилищеЗначения(Новый Структура);
	ПараметрыСеанса.РасширениеФоновыеЗаданияСтатус = "";
	ПараметрыСеанса.РасширениеФоновыеЗаданияИдентификаторЗадания = Новый УникальныйИдентификатор("00000000-0000-0000-0000-000000000000");
	ПараметрыСеанса.РасширениеФоновыеТекстОповещения = "";
	ПараметрыСеанса.РасширениеФоновыеЗаданияИнтервалОжидания = 1;
	ПараметрыСеанса.РасширениеФоновыеЗаданияКартинкаОповещения = "";
КонецПроцедуры
...
Процедура ПерезапуститьФоновоеЗадание() Экспорт
	ПараметрыСеансаРасш = ПараметрыСеансаРасширения().Параметры;
	Параметры = ПараметрыСеансаРасш.Параметры;
	ИдентификаторЗадания = ПараметрыСеанса.ИдентификаторЗадания;
	Задание = НайтиЗаданиеПоИдентификатору(ИдентификаторЗадания);
...
Функция ПроверитьЗаданиеВыполнено(ФормаЗакрывается = Ложь) Экспорт
	...
	ИдентификаторЗадания = ПараметрыСеанса.РасширениеФоновыеЗаданияИдентификаторЗадания;
	Параметры	= ПараметрыСеанса.РасширениеФоновыеЗаданияПараметры.Получить();
