...
Процедура Подключаемый_ПроверитьВыполнениеЗадания(ФормаЗакрывается = Ложь) Экспорт
	...
	ПараметрыСеансаРасширения = Saby_ФоновыеЗаданияСервер.ПараметрыСеансаРасширения();
	Параметры	= ПараметрыСеансаРасширения.Параметры;
	...
	Задание = Saby_ФоновыеЗаданияСервер.ПроверитьЗаданиеВыполнено(ФормаЗакрывается);
	Статус = Задание.Статус;
	Saby_ФоновыеЗаданияСервер.ПараметрыСеансаУстановитьСтатус(Статус);
	...
	Если Задание.Прогресс <> Неопределено Тогда
		...
		КонецЕсли;
	КонецЕсли;
	Saby_ФоновыеЗаданияСервер.ПараметрыСеансаУстановитьДополнительныеПараметры(ДополнительныеПараметры);
	...
	Если Статус = "Выполнено" Тогда
		...
		ИначеЕсли СтатусВыполнения = "error" Тогда
			...
			КонецЕсли;
			// 
			Saby_Core.ExtExceptionToJournal(РезультатВыгрузки);
		...
		Saby_ФоновыеЗаданияСервер.ПараметрыСеансаУстановитьКартинкуОповещения(КартинкаОповещения);
		...
		Если ТипЗнч(мСообщений) = Тип("Массив") Тогда
			...
			Saby_ФоновыеЗаданияСервер.ПараметрыСеансаУстановитьДополнительныеПараметры(ДополнительныеПараметры);
		КонецЕсли;
		Saby_ФоновыеЗаданияСервер.ПараметрыСеансаУстановитьТекстОповещения(ТекстОповещения);
		//Итоговое
		ПоказатьОповещение();
	...
	Если Параметры.ВыводитьОкноОжидания Тогда
		Если Параметры.Интервал = 0 Тогда
			...
			Если ИнтервалОжидания > 15 Тогда
				ИнтервалОжидания = 15;
			КонецЕсли;
			Saby_ФоновыеЗаданияСервер.ПараметрыСеансаУстановитьИнтервалОжидания(ИнтервалОжидания);
		КонецЕсли;
...
Процедура Подключаемый_ПроверитьВыполнениеЗаданияПослеВыполненияКоманды(commands_result, Контекст) Экспорт
	...
	Параметры = Saby_ФоновыеЗаданияСервер.ПараметрыСеансаРасширения().Параметры;
	...
	Saby_ФоновыеЗаданияСервер.ПерезапуститьФоновоеЗадание();
	Если Параметры.ВыводитьОкноОжидания Тогда
		Если Параметры.Интервал = 0 Тогда
			ИнтервалОжидания = ИнтервалОжидания * 1.4;
			Если ИнтервалОжидания > 15 Тогда
