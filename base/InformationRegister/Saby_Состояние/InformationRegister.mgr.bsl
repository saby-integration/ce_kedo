...
Процедура Обновить(UID, АктивныйЭтап="", Состояние=Неопределено, Объект, СбисИд) Экспорт
	...
	Иначе
		Состояние = Справочники.Saby_СостоянияОбъектов.НайтиПоРеквизиту("ИдентификаторСБИС", Число(СбисИд));
	КонецЕсли;
	...
	КонецЕсли;
	Если ЗначениеЗаполнено(СбисИд) ТОгда
...
Функция ПолучитьТикетТекущейСессии( context_params = Неопределено ) Экспорт
	...
	ОбработкаСБИС = Обработки.СБИС3.Создать();
...
#Область include_core_src_Helpers_НастройкиПодключения
#КонецОбласти  

#Область include_core_src_Helpers_ОбщиеНастройки
#КонецОбласти

#Область include_core_src_ОбработчикиСобытий_ПриПервомЗапуске
#КонецОбласти
