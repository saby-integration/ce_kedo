	...
	Попытка
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			Иначе
				...
				ЕстьФильтрФизЦица = Ложь;
				ЕстьФильтрПодразделения = Ложь;
				Для каждого ЭлементОтбора Из ОтчетОбъектРЛ.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы Цикл
					Если ТипЗнч(ЭлементОтбора) = Тип("ЭлементОтбораКомпоновкиДанных") Тогда
						...
						Если ТипЗнч(ЭлементОтбора.ПравоеЗначение) = Тип("СправочникСсылка.Организации") Тогда
							...
							КонецЕсли;   
							Если ПараметрыЗапроса.ЕстьФильтрПоОрганизации = Истина Тогда  
								ЭлементОтбора.Использование = Истина; 
							Иначе
							...
							КонецЕсли;
						КонецЕсли;
						Если ТипЗнч(ЭлементОтбора.ПравоеЗначение) = Тип("СправочникСсылка.ПодразделенияОрганизаций") Тогда
							Если ТипЗнч(ПараметрыЗапроса.Подразделения) = Тип("СправочникСсылка.ПодразделенияОрганизаций") Тогда
								ЭлементОтбора.ПравоеЗначение = ПараметрыЗапроса.Подразделения;
							Иначе
								ЭлементОтбора.ПравоеЗначение = ВыборкаДетальныеЗаписи.Подразделение;
							КонецЕсли; 
							Если ПараметрыЗапроса.ЕстьФильтрПоПодразделениям = Истина Тогда
								ЭлементОтбора.Использование = Истина;
							Иначе
								ЭлементОтбора.Использование = Ложь;
							КонецЕсли;
							Если ПараметрыЗапроса.Свойство("ВидСравненияПодразделения") Тогда
								ЭлементОтбора.ВидСравнения = ПараметрыЗапроса.ВидСравненияПодразделения;
							Иначе
								ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
							КонецЕсли;
							ЕстьФильтрПодразделения = Истина;
						КонецЕсли;
						Если ТипЗнч(ЭлементОтбора.ПравоеЗначение) = Тип("СписокЗначений") Тогда 
							Если ЭлементОтбора.ПравоеЗначение.Количество() > 0 Тогда  
					...
					КонецЕсли;
				КонецЦикла; 
				Если ЕстьФильтрФизЦица = Ложь Тогда 
					Для каждого ЭлементОтбора Из ОтчетОбъектРЛ.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы Цикл
						Если ТипЗнч(ЭлементОтбора) = Тип("ЭлементОтбораКомпоновкиДанных") Тогда
							Если ЭлементОтбора.ПравоеЗначение = Неопределено Тогда
					        	Если ЭлементОтбора.Использование = Ложь Тогда
									...
									ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
									ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ФизическиеЛица");;
									ЭлементОтбора.ПравоеЗначение = ВыборкаДетальныеЗаписи.ФизическоеЛицо;
									ЭлементОтбора.Использование = Истина;
								КонецЕсли;
							КонецЕсли;
					...
					КонецЦикла;
				КонецЕсли;
				Если ЕстьФильтрПодразделения = Ложь И ПараметрыЗапроса.ЕстьФильтрПоПодразделениям = Истина Тогда 
					Для каждого ЭлементОтбора Из ОтчетОбъектРЛ.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы Цикл
						Если ТипЗнч(ЭлементОтбора) = Тип("ЭлементОтбораКомпоновкиДанных") Тогда
							Если ЭлементОтбора.ПравоеЗначение = Неопределено Тогда
					        	Если ЭлементОтбора.Использование = Ложь Тогда
									ЭлементОтбора.ИдентификаторПользовательскойНастройки = Новый УникальныйИдентификатор();
									ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
									ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Подразделения");
									ЭлементОтбора.ПравоеЗначение = ВыборкаДетальныеЗаписи.Подразделение;
									ЭлементОтбора.Использование = Истина;
								КонецЕсли;
							КонецЕсли;
